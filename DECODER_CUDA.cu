 ...
 */
__device__ int rs_decode_palindromic_CUDA(const unsigned char* Sr, unsigned char* S, int syndrome_cnt,
    ...
    unsigned char* Lambda, unsigned char* B, unsigned char* T, unsigned char* Omega)
{
    ...
    for (int i = 0; i < syndrome_cnt; i++)
    {
        ...
        {
            S[i] ^= Sr[j];
            S[i] = gf_mul_CUDA(S[i], gf_exp_CUDA[tf0]);
        }
    }
    ...
    for (int r = 0; r < syndrome_cnt; r++) {
        unsigned char d = S[r];
        for (int i = 1; i <= L; i++) {
            d ^= gf_mul_CUDA(Lambda[i], S[r - i]); // Galois field addition is XOR
        }
        ...
        if (d == 0) {
            ...
            unsigned char scale = gf_div_CUDA(d, b);

            // Update Error Locator: Lambda(x) = Lambda(x) ^ (scale * x^m * B(x))
            for (int i = 0; i <= syndrome_cnt; i++) {
                if (i + m <= syndrome_cnt) {
                    Lambda[i + m] ^= gf_mul_CUDA(scale, B[i]);
                }
            }
    ...
    for (int i = 0; i < L; i++) {
        unsigned char sum = 0;
        for (int j = 0; j <= i; j++) {
            sum ^= gf_mul_CUDA(S[i - j], Lambda[j]);
        }
        Omega[i] = sum;
    }
    ...
    for (int i = 0; i < codeword_len; i++) {
        ...
        unsigned char inv_root = gf_exp_CUDA[target_exp];
        ...
        for (int j = 0; j <= L; j++) {
            ...
            if (j > 0 && inv_root != 0) {
                int term_exp = (gf_log_CUDA[inv_root] * j) % 255;
                if (term_exp < 0) term_exp += 255;
                term_pow = gf_exp_CUDA[term_exp];
            }
            else if (j > 0) {
                term_pow = 0;
            }
            lambda_val ^= gf_mul_CUDA(Lambda[j], term_pow);
        }
        ...
        if (lambda_val == 0) {
            ...
            for (int j = 0; j < L; j++) {
                ...
                omega_val ^= gf_mul_CUDA(Omega[j], term_pow);
            }
            ...
            for (int j = 1; j <= L; j += 2) { // Even indices cancel out to 0 under GF(2)
                ...
                if (deriv_pow > 0 && inv_root != 0) {
                    int term_exp = (gf_log_CUDA[inv_root] * deriv_pow) % 255;
                    if (term_exp < 0) term_exp += 255;
                    term_pow = gf_exp_CUDA[term_exp];
                }
                else if (deriv_pow > 0) {
                ...
                lambda_deriv ^= gf_mul_CUDA(Lambda[j], term_pow);
            }
            ...
            if (scale_exp != 0 && inv_root != 0) {
                int term_exp = (gf_log_CUDA[inv_root] * scale_exp) % 255;
                if (term_exp < 0) term_exp += 255;
                num_scaling = gf_exp_CUDA[term_exp];
            }
            else if (scale_exp != 0) {
                num_scaling = 0;
            }

            unsigned char numerator = gf_mul_CUDA(omega_val, num_scaling);
            err_mags[error_idx] = gf_div_CUDA(numerator, lambda_deriv);
            error_idx++;
    ...
    for (int i = 0; i < L; i++)
    {
        ...
        printf("GPU Correcting buffer %d offset %d with %x\n", idx, boff, err_mags[i]);
        unsigned char* bufloc = (unsigned char*)CWP[idx];
        bufloc += boff;
        *bufloc ^= err_mags[i];
    }
